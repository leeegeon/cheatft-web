...
검증은 Codex가 변경 위험도와 영향 범위를 판단해 필요할 때만 실행한다. 단순 문구 변경, 문서 정리, 작은 UI 배치 수정처럼 정적 확인으로 충분한 작업은 lint/test/build를 생략할 수 있고, 생략한 경우 최종 답변에 실행하지 않았다고 명시한다.

동작 로직, API 연동, 인증 흐름, 라우팅, 공용 유틸, 빌드 설정, 의존성, 여러 화면에 영향을 주는 CSS를 바꿨거나 회귀 위험이 있다고 판단되면 `cheatft_web`에서 필요한 범위의 명령을 골라 확인한다.
