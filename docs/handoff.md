...
- 홈: `GET /summary`의 `todayStats`, `recentChecks`를 표시하고 프론트 더미 fallback은 쓰지 않는다. 2026-08-05 배포 API 확인 기준 `reliabilityStatus`/`biasStatus.categories`는 응답에 없어 홈 신뢰도 현황은 점수 `-`, 상태 `확인중`, 빈 상태로 보일 수 있다. 모바일에서는 검색창과 `신뢰도 분석` 버튼이 가로폭을 꽉 쓰도록 보정했다.
