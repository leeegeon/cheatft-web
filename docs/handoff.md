...
- 리포트 상세 탭은 관련 기사/반박 기사만 두고, 오른쪽 종합 요약 패널은 항상 보여준다. 상세 기사에 날짜가 없으면 `-` 문자를 표시하지 않는다. 상세 기사에 `url/link/originalLink`가 있으면 제목을 원문 새 탭 링크로 표시하고, URL이 없으면 기사 제목과 언론사 기준 네이버 뉴스 검색 링크로 표시한다.
