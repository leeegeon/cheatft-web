...
- 화면 데이터: 홈/신뢰도 분석(`/search`)/편향성 분석(`/algo`)/팩트체크 리포트/커뮤니티는 프론트 더미 fallback을 제거하고 서비스 응답 기반 데이터/오류/빈 상태만 표시한다.
- 화면 문구: `백엔드 API`, `API 응답 표시 중`, `분석 전` 같은 개발자용 출처/상태 문구는 표시하지 않고, 로딩/오류/빈 상태를 사용자용 문구로 안내한다.
...
- 홈: `GET /summary`의 `todayStats`, `recentChecks`를 표시하고 프론트 더미 fallback은 쓰지 않는다. 2026-08-05 작업에서 응답 부재로 비어 보이던 홈 신뢰도 현황 블록은 제거했고, 오른쪽 하단에는 `신뢰할 수 있는 정보, 모두의 책임입니다.` 책임 메시지 카드를 표시한다. 모바일에서는 검색창과 `신뢰도 분석` 버튼이 가로폭을 꽉 쓰고, 책임 메시지 카드는 오른쪽 열 높이 맞춤을 해제해 자연 높이로 표시한다.
- 뉴스 상세: 신뢰도 분석 카드 클릭으로 전달된 기사 객체를 route state/sessionStorage에서 먼저 표시하고, 지원되는 네이버 기사 URL이 있으면 `/mnews/article/`을 `/article/`로 정규화한 뒤 `POST /article` 상세 API 응답의 `content`를 기사 전문 영역에 우선 표시한다. 상세 API가 실패하면 오류 문구 없이 목록 기사 정보를 유지한다. 현재 배포 API가 `content`를 축약 문자열로 내려주면 화면에도 축약 본문까지만 표시된다. 뉴스 상세 상단 메타에서는 조회수를 표시하지 않는다. 신뢰도 점수는 백엔드/목록 값을 우선 사용하고 없으면 `src/data/pressReliability.js`의 언론사 기준표를 사용하며, 오른쪽 패널은 0~5 눈금과 현재 점수 마커로 표시한다.
...
- 커뮤니티 목록: 카테고리 이동은 왼쪽 세로 메뉴만 사용하고, 중복 상단 가로 탭과 카테고리 select는 제거됐다. 카테고리 아래에는 `커뮤니티 참여 현황`을 3칸 가로형으로 작게 표시한다. 목록 API 응답에는 인기 태그 목록이 없어 하드코딩 인기 태그 영역도 제거했다. `최신순`, `인기순`, `댓글 많은 순` 버튼은 수신한 게시글을 프론트에서 정렬한다. `최근 정정 요청`은 현재 탭과 별개로 `GET /posts?category=정정 요청`을 따로 조회해 표시한다. 게시글 카드 오른쪽 위 북마크 장식은 제거했고, 오른쪽 보조 영역의 `인기 게시글`, `최근 정정 요청`은 같은 번호 배지/제목/조회·댓글 메타 양식으로 표시한다. 2026-08-05 배포 API 확인 기준 `communityStats.todayComments`는 게시글별 `commentCount`가 있어도 0을 반환한다.
...
- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- 후속 뉴스 상세/신뢰도 분석 조정으로 기사 상세는 지원되는 네이버 기사 URL이 있으면 `POST /article` 전용 API 응답을 우선 병합해 `content` 본문/기자/입력 시간/주제를 표시한다.
- 상세 API 응답은 URL별 `sessionStorage` 캐시에 저장해 같은 탭에서 같은 기사 상세 재진입 시 재요청을 줄인다.
- 신뢰도 분석 검색 결과는 검색어별 `sessionStorage` 캐시에 저장해 기사 상세에서 뒤로가기 했을 때 `POST /checks` 재요청과 긴 로딩 팝업을 반복하지 않고 기존 결과를 즉시 복원한다.
- 후속 프론트 조정으로 편향성 분석 질문창은 Enter로 키워드 추천을 실행하고, Shift+Enter는 줄바꿈으로 유지한다.
- 키워드 추천 중에는 추천 버튼에 스피너를 표시한다.
- 분석 완료 후 메인 제목은 `'키워드' 분석 결과` 형식으로 표시한다.
- 편향성 분석 기사 배지와 하단 집계는 백엔드 `stance`/count 값인 `긍정`, `중립`, `반박`을 그대로 표시한다.
- 편향성 분석의 정보 편향성 패널은 긍정/중립/반박 건수 위에 각 비율을 가로 막대그래프로 표시하고, 막대와 건수 카드에 퍼센트를 함께 보여준다.
- 분석 기사 항목에 `url/link/originalLink`가 있으면 기사 카드를 원문 새 탭 링크로 렌더링한다. 현재 운영 API처럼 URL이 없으면 기사 제목과 언론사 기준 네이버 뉴스 검색 링크로 렌더링한다.
...
- 리포트 목록은 `GET /reports`에 `page`와 `limit=10`을 전달하고, 백엔드 `pagination`으로 이전/다음/페이지 번호 이동을 제공한다. 검색어/기간/신뢰도 필터 변경과 페이지 이동 시 펼친 상세는 닫는다.
...
- 리포트 상세 탭은 관련 기사/반박 기사만 두고, 오른쪽 종합 요약 패널은 항상 보여준다. 상세 보기/접기 버튼 문구에는 화살표를 쓰지 않고 버튼은 카드 오른쪽에 유지한다. 상세 기사 상태 배지는 클릭 가능한 커서로 보이지 않게 한다. 상세 기사에 날짜가 없으면 `-` 문자를 표시하지 않는다. 상세 기사에 `url/link/originalLink`가 있으면 제목을 원문 새 탭 링크로 표시하고, URL이 없으면 기사 제목과 언론사 기준 네이버 뉴스 검색 링크로 표시한다.
...
- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- 운영 API에 테스트 계정으로 로그인한 뒤 `POST /api/analysis`, `GET /api/analysis/{id}?limit=4`를 직접 호출해 실제 응답을 확인했다.
- 실제 응답은 `analysisId`, `keyword`, `biasAnalysis.positiveCount/neutralCount/negativeCount/biasScore`, `insights`, `relatedArticles`, `counterArticles`, `summaryStats`, `pagination` 구조다.
...
- 신뢰도 게이지의 호 위 라벨/건수 텍스트가 겹쳐 보이는 문제를 줄이기 위해 SVG 내부 텍스트를 제거하고, 중앙 점수/등급과 하단 긍정/중립/반박 미니 카드로 분리했다.
- 추가 확인에서 잘못된 Bearer token으로 `POST /api/analysis`를 호출하면 운영 API가 `403 유효하지 않은 토큰입니다.`를 반환하는 것을 확인했다. 이 경우 프론트는 실패 상태를 반복 표시하지 않고 세션을 정리한 뒤 로그인 화면으로 이동한다.
- 검증: `npm run lint`, `npm test`, Codex 번들 Node 기반 `npm run build` 통과. 기본 셸 `npm run build`는 기존 Node/Vite 네이티브 이슈로 `43 modules transformed` 뒤 실패했다.
...
- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- `파일/신뢰도`의 이미지 자료를 확인해 AI 별점은 참고값으로만 사용하고, Codex가 언론사별 분류/신뢰도/판단 이유를 새로 정리했다.
- 사이트 반영 원본은 `src/data/pressReliability.js`이다. 79개 언론사에 대해 `category`, `reliabilityScore`, `reliabilityLabel`, `aiReferenceStars`, `rationaleSummary`를 기록했다.
- 이유 확인용 문서는 `docs/press-reliability.md`로 생성했다.
- `src/utils/press.js`는 `pressReliability.js`를 기준으로 언론사 분류와 신뢰도를 조회한다. 백엔드 `PRESS_MAPPING`에는 있으나 프론트에 없던 `144: 스포츠경향`도 추가했다.
- `VerificationView.jsx`는 백엔드 기사에 신뢰도 점수가 없으면 언론사 기준 신뢰도를 표시하고, 기사 상세로 전달한다.
- `DetailView.jsx`는 전달받은 언론사 신뢰도 분류와 판단 이유 요약을 오른쪽 신뢰도 패널에 표시한다.
- `AlgoView.jsx`, `ReportView.jsx`도 언론사 배지/주요 출처 점수에 같은 기준표를 fallback으로 사용한다.
- 배포 API `POST /api/checks` 후 `GET /api/checks/{id}?page=1&limit=100`을 직접 확인했다.
  - `인공지능 뉴스`, `정치`, `경제`, `대통령` 검색어 모두 `totalArticles: 12`, `articles.length: 12`, `pagination.totalPages: 1`로 관측됐다.
  - `page=2&limit=5` 요청도 12건 전체와 `totalPages: 1`을 반환해, 현재 배포 API에서는 서버 페이지네이션이 아직 적용되지 않은 상태로 보인다.
...
- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- `src/components/views/AlgoView.jsx`에서 질문 영역과 추천 키워드 영역을 각각 독립된 강조 박스로 나눴다.
- 기본 상태와 키워드 칩 선택 직후에는 질문 영역을 강조한다.
- `키워드 추천`으로 추천 키워드가 생성되면 키워드 영역을 강조한다. 추천 키워드가 없으면 질문 영역 강조를 유지한다.
...
- 배포 API를 실제 호출해 `cheatft_web` 문서의 현재 상태 문구를 갱신했다.
- `GET /api/summary`는 `recentChecks` 3개를 반환한다.
- `GET /api/reports`, `GET /api/posts`는 query parameter를 받아도 같은 dummy 응답과 `currentPage: 1`을 반환한다.
- `GET /api/checks/{id}`는 `page=2&limit=5`에도 전체 12건과 `currentPage: 1`을 반환해 서버 페이지네이션이 아직 적용되지 않은 상태로 보인다. article `press`는 숫자가 아니라 언론사명 문자열이며, 제목/설명에는 HTML entity가 남을 수 있다.
- `POST /api/checks`의 `type=url`은 202로 접수되지만 URL 본문 파싱이 아니라 검색어처럼 처리되며, 확인한 네이버 기사 URL은 기사 0건을 반환했다.
- `GET /api/analysis/{id}`는 `limit=1`과 `limit=4`에 같은 결과를 반환하고 응답 body에 `limit` 필드가 없다.
- `POST /api/signup` 중복 이메일은 현재 `500`, `이미 사용 중인 이메일입니다.` 메시지를 반환한다.
- `GET /api/health`는 공통 응답 래핑 없이 `{ message }`만 반환한다.
- `POST /api/posts`는 성공 시 `id`, `title`, `category`만 반환한다.
...
- 배포 API 확인 결과:
  ...
  - `/api/me`는 토큰 없으면 401.
  - 당시 `/api/login`은 `UserModel.findByEmail is not a function` 메시지와 함께 401 응답. 로컬 `cheatft_api/src/models/user.model.js`가 사용자 모델이 아니라 checks 모델 내용으로 들어간 상태와 일치했다. 이후 2026-07-16/2026-07-26 확인 기준 배포 로그인은 정상 동작한다.
...
- 배포 실패 원인 확인:
  ...
  - 당시 `/api/login`은 `UserModel.findByEmail is not a function` 오류를 반환했다. 이후 2026-07-16/2026-07-26 확인 기준 배포 로그인은 정상 동작한다.
- UI 문구/구조 변경:
...
- 홈/검증하기 실데이터 전환:
  ...
  - 홈 최신 팩트체크는 `recentChecks.slice(0, 3)` 제한 없이 백엔드가 주는 항목을 모두 표시한다. 2026-07-26 배포 `/api/summary` 확인 기준 `recentChecks`는 3개다.
...
- 2026-07-10 당시 확인한 `/summary`의 `recentChecks`는 1개였다. 2026-07-26 재확인 기준은 3개다.
- 2026-07-10 당시 `POST /checks`는 `checkId: 452`를 반환했고, `GET /checks/452`는 기사 배열 `articles`에 1개 기사를 반환했다. 2026-07-26 기준 새 DB 기반 응답에서는 새로 생성한 check id를 조회해야 한다.
...
- 정렬 select는 과거 `최신순`, `조회수순`, `연관도순`을 제공했다. 2026-07-26 이후 현재 검증하기 정렬은 `연관도순`, `최신순`만 제공한다.
...
- 프론트는 회원가입 실패 시 `409` 이메일/닉네임 중복 안내를 처리하고, 성공 시 로그인 화면으로 이동한다. 2026-07-26 배포 API 확인 기준 중복 이메일은 현재 `500`으로 내려온다.
...
- 인증 메모:
  ...
  - 이 인증 메모는 2026-07-15 당시 기록이다. 2026-07-16/2026-07-26 확인 기준 배포 `/api/login`, `/api/me`는 테스트 계정으로 정상 동작한다.
- 검증:
  - `npm run lint` 통과
  - `npm test` 통과
  - Codex 번들 Node 기반 `npm run build` 통과
