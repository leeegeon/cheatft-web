...
## 2026-08-05 서비스 화면 문구 정리

- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- 홈 신뢰도 현황 블록은 제거하고, 오른쪽 영역에 `신뢰할 수 있는 정보, 모두의 책임입니다.` 책임 메시지 카드를 표시한다. 모바일에서는 검색창과 `신뢰도 분석` 버튼이 가로폭을 꽉 쓰도록 보정했다.
- 홈/신뢰도 분석/뉴스 상세/편향성 분석/팩트체크 리포트/커뮤니티/인증/글쓰기 화면에서 `백엔드 API`, `API 응답 표시 중`, `분석 전`, `API 게시글`, `상세 API 반환 본문` 같은 개발자용 출처/상태 문구를 제거하고 사용자용 로딩/오류/빈 상태 문구로 바꿨다.
- `src/services/apiClient.js`의 API 설정 누락 에러도 `VITE_API_BASE_URL` 문자열이 화면에 노출되지 않도록 사용자용 문구로 변경했다.
- 검증:
  - `npm run lint` 통과.
  - `npm test`, `npm run build`는 화면 문구/배치 정리 중심이라 실행하지 않았다.
...
- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- `AlgoView.jsx`는 분석 기사에서 `url/link/originalLink`를 매핑하고 URL이 있으면 카드 전체를 원문 링크로 연다. 링크가 있더라도 제목은 기존 일반 텍스트 스타일을 유지한다.
- `ReportView.jsx`의 리포트 상세 기사 매핑에 `url/link/originalLink/originallink` 후보를 추가하고, URL이 있으면 기사 제목을 원문 새 탭 링크로 렌더링한다.
- 배포 API 확인 기준 최근 리포트 5개 모두 분석 상세 기사 `url` 값이 `null`이었다. 이 경우 기사 제목과 언론사명으로 네이버 뉴스 검색 URL을 만들어 링크로 표시한다.
- 검증:
  - `npm run lint` 통과.
  - `npm test` 통과.
  - Codex 번들 Node 우선 PATH에서 `npm run build` 통과.

## 2026-08-05 커뮤니티 목록 UI 정리

- 백엔드 폴더(`cheatft_api`)는 수정하지 않았다.
- `GET /api/posts` 계약은 `communityStats`, `posts`, `pagination` 중심이며 인기 태그 목록을 반환하지 않으므로, `CommunityView.jsx`의 하드코딩 인기 태그 영역을 제거했다.
- 커뮤니티 카테고리 이동은 왼쪽 세로 메뉴만 남기고 중복 상단 가로 탭과 카테고리 select를 제거했다.
- `최신순`, `인기순`, `댓글 많은 순` 버튼을 실제 상태로 연결해 API에서 받은 게시글 배열을 작성일, 조회수, 댓글 수 기준으로 프론트 정렬한다.
- `커뮤니티 참여 현황`은 왼쪽 카테고리 메뉴 아래로 옮겨 3칸 가로형으로 정리했다. 좁은 왼쪽 영역에서도 줄바꿈이 지저분하지 않도록 라벨과 숫자 크기를 줄였다.
- `최근 정정 요청`은 현재 탭 목록에 의존하지 않고 `GET /posts?category=정정 요청`을 별도로 조회해 표시한다.
- 게시글 카드 오른쪽의 큰 분류 아이콘 박스는 제거했다.
- 오른쪽 `인기 게시글`, `최근 정정 요청`, `정보 오류` 카드는 기본 크기감을 맞췄고, 중간 폭에서는 카드 그리드, 모바일 폭에서는 1열로 전환한다.
- 검증:
  - `npm run lint` 통과.
  - `npm test` 통과.
  - 기본 셸 `npm run build`는 기존 Node/Vite 이슈처럼 transform 후 exit 1.
  - Codex 번들 Node 우선 PATH에서 `npm run build` 통과.
  - dev server는 기존 `3001` 사용으로 `http://127.0.0.1:3002/`에 열렸고 `/community` 응답 `200` 확인.
...
- 후속 편향성 분석 UX 조정:
  ...
  - 분석 기사 응답에 `url/link/originalLink`가 있으면 카드 클릭 시 원문을 새 탭으로 연다. 2026-08-05 후속 보강으로 URL이 없으면 기사 제목과 언론사명 기반 네이버 뉴스 검색 링크를 연다.
