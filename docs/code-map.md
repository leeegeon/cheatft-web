...
- `AlgoView.jsx`
  ...
  - 기사 변환에서 `articleId`, `press`, `title`, `stance`를 우선 처리한다. 기사 배지는 백엔드 `stance` 값인 `긍정`, `중립`, `반박`을 그대로 표시한다. `url/link/originalLink`가 있으면 카드 클릭 시 원문을 새 탭으로 열고, 현재 운영 API처럼 URL이 없으면 기사 제목과 언론사 기준 네이버 뉴스 검색을 연다.
...
- `CommunityView.jsx`
  - `tab` query param으로 왼쪽 세로 카테고리 메뉴 상태를 공유한다. 중복 상단 가로 탭과 카테고리 select는 제거됐다.
  ...
  - `글 작성하기` 버튼은 전역 상단바가 아니라 커뮤니티 목록 상단의 검색 영역에 표시한다.
